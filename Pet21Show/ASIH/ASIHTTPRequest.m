...
- (void)main
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [[NSFileManager defaultManager] fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		// For iPhone 2.0 and later
		[self setPartialDownloadSize:[[[NSFileManager defaultManager] attributesOfItemAtPath:[self temporaryFileDownloadPath] error:nil] fileSize]];
...
// compress txt data with zlib
+ (NSData *)compressTxtData:(NSData*)decompressedData
{
	if ([decompressedData length] == 0) return decompressedData;
	
	unsigned full_length = [decompressedData length];
	unsigned half_length = [decompressedData length] / 2;
	...
	z_stream strm;
	strm.next_in = (Bytef *)[decompressedData bytes];
...
// compress txt data from a file into another file, used when update to a file
+ (int)compressTxtDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath
{
	// Get a FILE struct for the source file
	FILE *source = fdopen([[NSFileHandle fileHandleForReadingAtPath:sourcePath] fileDescriptor], "r");
	...
	// Uncompress data in source and save in destination
	int status = [ASIHTTPRequest compressTxtDataFromSource:source toDestination:dest];
...
+ (int)compressTxtDataFromSource:(FILE *)source toDestination:(FILE *)dest
{
	int ret, flush;
    unsigned have;
    z_stream strm;
...
// compress txt data from a file into another file, used when update to a file
+ (int)compressDataFromFileToDestFile:(NSString *)sourcePath toFile:(NSString *)destinationPath
{
	// Get a FILE struct for the source file
	FILE *source = fdopen([[NSFileHandle fileHandleForReadingAtPath:sourcePath] fileDescriptor], "r");
	...
	// Get a FILE struct for the destination path
	FILE *dest = fdopen([[NSFileHandle fileHandleForUpdatingAtPath:destinationPath] fileDescriptor], "a"); 
